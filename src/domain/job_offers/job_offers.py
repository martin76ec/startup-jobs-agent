...
class JobOfferStruct(BaseModel):
  ...
  details: str = Field(default="", description="the details in markdown about the job")
  name: str = Field(default="", description="the role of the position")
  remote: str = Field(default="Remote", description="the type of job, Remote, Hybrid or On-Site")
  startup: str = Field(default="", description="the name of the company that makes the offer")
  vertical: Literal[
    "Admin",
    "Backend",
    "Clouding",
...
def job_raw_to_obj(raw_job: Dict[str, Any]) -> JobOffer:
  return JobOffer(
    ...
    date_scrapped=raw_job.get("properties", {}).get("Date Scrapped", {}).get("created_time"),
    details=raw_job.get("properties", {}).get("Details", {}).get("rich_text", [{}])[0].get("plain_text"),
    name=raw_job.get("properties", {}).get("Name", {}).get("title", [{}])[0].get("plain_text"),
    remote=raw_job.get("properties", {}).get("Remote", {}).get("select", {}).get("name"),
    startup=raw_job.get("properties", {}).get("Startup", {}).get("rich_text", [{}])[0].get("plain_text"),
